...
export class SellerSignUpDto {
  @IsString()
  @MaxLength(50)
  @ApiProperty({name:"seller_name",description:"seller_name",type:"string",required:true})
  seller_name: string;
  ...
  @MaxLength(50)
  @Matches(/((?=.*d)|(?=.*\W+))(?![.\n])(?=.*[A-Z])(?=.*[a-z]).*$/)
  @ApiProperty({name:"seller_password",description:"seller_password",type:"string",required:true})
